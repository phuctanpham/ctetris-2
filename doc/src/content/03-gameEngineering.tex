...
\begin{tikzpicture}[class/.style={rectangle, draw=blue!80, fill=blue!6, rounded corners=2pt, align=left, font=\scriptsize\ttfamily, inner sep=4pt}]
  \node[class] (dn) at (0,0) {
    ...
    \color{gray}\rule{3.2cm}{0.4pt}\\[2pt]
    + maNut: int\\
    + tenNhanVat: string\\
    + noiDungThoai: string\\
    + duongDanAnh: string\\
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[
  b/.style={rectangle,draw=blue!80,fill=blue!8,rounded corners=2pt,text width=3.4cm,align=center,font=\scriptsize,minimum height=0.6cm},
  ...
  \draw[arr] (ins.south) |- (out.west); \draw[arr] (intro.south) |- (out.east);
\end{tikzpicture}
\end{adjustbox}
\end{center}
...
\begin{tikzpicture}[class/.style={rectangle, draw=blue!80, fill=blue!6, rounded corners=2pt, align=left, font=\scriptsize\ttfamily, inner sep=4pt}]
  ...
  \node[class, anchor=north] (sr) at (4.5,0) {
  ...
  \node[class, anchor=north] (be) at (9.6,0) {
...
\begin{tikzpicture}[every node/.style={font=\scriptsize},x=1.1cm,y=0.65cm]
  ...
  \foreach \x/\c in {0/white!70!gray,1/gray!80,2/white!70!gray, 3/gray!80,4/white!70!gray,5/gray!80}{
    \fill[\c](\x,0.3) rectangle (\x+1,0.8); \draw[black](\x,0.3) rectangle (\x+1,0.8);
  }
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[
  proc/.style={rectangle,draw=blue!80,fill=blue!8,rounded corners=2pt, minimum width=2.6cm,minimum height=0.6cm,align=center,font=\tiny},
  store/.style={cylinder,draw=gray!60,fill=gray!10,shape border rotate=90, minimum width=1.8cm,minimum height=0.5cm,align=center,font=\tiny},
  extio/.style={ellipse,draw=gray!50,fill=gray!6,minimum width=2.0cm, minimum height=0.5cm,align=center,font=\tiny},
  ...
  \node[extio](cfg2)  at (6.5,5.5) {CauHinhCaiDat};
  \node[proc](route)  at (3.25,5.5){Bộ định tuyến Sự kiện};
  \node[proc](phys2)  at (3.25,3.5) {Cập nhật Vật lý};
  \node[store](gs2)   at (6.5,3.5)  {TrangThaiLoi};
  \node[proc](flash2) at (3.25,1.5) {Hiệu ứng Chớp};
  \node[proc](rend3)  at (0,1.5)    {Tầng Hiển thị};
  \node[proc](ogo)    at (3.25,-0.5){\texttt{XuLyKetThucVan()}};
  \node[store](db3)   at (6.5,-0.5) {CSDL Cục bộ};
