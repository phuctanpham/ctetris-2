...
\noindent\fbox{\begin{minipage}{0.95\linewidth}
\textit{Tham khảo kỹ thuật:} swipe gesture decision tree
(threshold 15px, trục ưu thế, swipeOnBlock vs board zone),
checkCollision formula (matrix-based O(4)), và rotation pivot
\texttt{(px=1,py=1)} được đặc tả trong
\textbf{SDS §07-gameCoreEngineering, mục 2.3.2 và 2.3.6}.
\end{minipage}}
\vspace{6pt}
...
\noindent\fbox{\begin{minipage}{0.95\linewidth}
\textit{Tham khảo kỹ thuật:} \texttt{getFallInterval()} decision tree (5 thresholds),
softDrop = interval$\div$5 (dynamic), và elapsed time formula
(pause-excluded: \texttt{nowMs - gameStartTime - totalPausedMs})
được đặc tả trong \textbf{SDS §07-gameCoreEngineering, mục 2.3.4 và 2.3.5}.
\end{minipage}}
\vspace{6pt}
...
\noindent\fbox{\begin{minipage}{0.95\linewidth}
\textit{Tham khảo kỹ thuật:} Flash clear sequence flowchart
(lockBlock $\rightarrow$ pendingClear $\rightarrow$ 6-tick flash loop
$\rightarrow$ finishLineClear), timing diagram, và combo score
formula ($\Delta\text{score} = n^2$) được đặc tả trong
...
\noindent\fbox{\begin{minipage}{0.95\linewidth}
\textit{Tham khảo kỹ thuật:} DB write boundary diagram (gameCore không sở hữu
schema), sequence diagram của onGameOver() (dbOpen $\rightarrow$ INSERT
$\rightarrow$ UPSERT $\rightarrow$ cascade $\rightarrow$ dbClose),
\texttt{gameOverRecorded} idempotency guard, và GameRecord field mapping
được đặc tả trong \textbf{SDS §07-gameCoreEngineering, mục 2.4 và 3.2}.
\end{minipage}}
\vspace{6pt}
