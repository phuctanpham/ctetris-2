...
\subsubsection{Post-sync Condition Check --- Return Value Contract}
...
\begin{tikzpicture}[
  ...
  % shared_data → shared_dialogues (1:N on idStory+idChapter)
  \draw[rel](sd.north east)to[bend left=10]
      node[lbl,pos=0.3]{1}node[lbl,pos=0.75]{N}(dlg.west);
  % shared_dialogues → shared_choices (1:N on idStory+idChapter+nodeId)
  \draw[rel](dlg.south)--node[lbl,left,pos=0.4]{1}node[lbl,pos=0.85]{N}(cho.north);
  % shared_data → shared_choices direct FK (idStory+idChapter)
...
// Empty string → auto SYNC_OFFLINE fallback, no crash.
...
\begin{itemize}[nosep]
  ...
  \item \texttt{MANIFEST\_GIST\_URL=""} $\rightarrow$ \texttt{SYNC\_OFFLINE}; no crash;
...
\texttt{SDL\_GetPrefPath("uit","cTetris") + "cache/" + \textit{basename}(url)}.
