...
\begin{tikzpicture}[
  ...
  \node[dec] (d0) at (0,-1.3) {body\\empty?};
  \node[term](off)at (4.5,-1.3){\texttt{SYNC\_OFFLINE} $\rightarrow$ return};
  \node[proc](p1) at (0,-2.6) {parse JSON $\rightarrow$ \texttt{versions[latest]}};
  \node[proc](p2) at (0,-3.7) {foreach chapter: \texttt{metaGetSha()} vs newSha};
  \node[dec] (d1) at (0,-5.0) {SHA\\match?};
  \node[term](sk) at (4.5,-5.0){skip $\rightarrow$ next chapter};
  \node[proc](p3) at (0,-6.3) {\texttt{httpGetSync(gistUrl)} $\rightarrow$ JSON body};
  \node[proc](p4) at (0,-7.4) {\texttt{applyChapterJson(db, body, chapterId)}};
  \node[proc](p5) at (0,-8.5) {\texttt{metaSetSha()}; WASM: \texttt{FS.syncfs()}};
  \node[wterm](dn)at (0,-9.6) {\texttt{SYNC\_DONE}};
  \draw[arr](p0)--(d0);
...
% loop-back arrow for chapters
  \draw[arr,gray,dashed](sk.east) -- ++(0.6,0) |- node[pos=0.25,right,font=\tiny]{next ch} (p2.east);
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[node distance=1.7cm,
  ...
  % Fixed arrow: Explicitly leaves chk.north going up-left (145 deg) 
  % and enters wait.north coming from the up-right (35 deg)
  \draw[sdsArrow=pillarBlue] (chk.north) to[out=145, in=35]
      node[above,font=\tiny]{curIdx=nextIdx} (wait.north);
...
\begin{tikzpicture}[
  ...
  \node[font=\scriptsize\itshape, text=gray, anchor=south west] at (dn.north west) {gameStory\_db.h (public)};

  \node[class] (dc) at (5.0, 0.4) {
    \textbf{DialogueChoice}\\[2pt]
    \color{gray}\rule{3.2cm}{0.4pt}\\[2pt]
    + choiceIdx: int\\
    + label: string\\
    + nextNodeId: int
  ...
  \node[font=\scriptsize\itshape, text=gray, anchor=south west] at (svg.north west) {app.cpp (module-internal)};

  \node[class] (sp) at (5.0, -4.0) {
    \textbf{SyncProgress}\\[2pt]
    \color{gray}\rule{3.2cm}{0.4pt}\\[2pt]
    + total: int\\
    + done: int\\
    + currentId: string\\
  ...
  \node[enum] (ss) at (9.6, -4.0) {
    \textbf{\guillemotleft enum\guillemotright~SyncState}\\[2pt]
    \color{gray}\rule{3.2cm}{0.4pt}\\[2pt]
    SYNC\_IDLE\\
    SYNC\_FETCHING\\
    SYNC\_UPDATING\_DB\\
 ...
 \draw[-{Stealth[length=4pt]}, thick, dashed, gray] (sp.east) -- (ss.west);

  % Center short breaking line (Horizontally centered around x=4.8, pushed down to y=-7.5)
  \draw[thick, dashed, gray!50] (2.8, -7.5) -- (6.8, -7.5);

  % logger.h (shared) - Shifted left and down
  \node[class, fill=versionAmber!6, draw=versionAmber] (logger) at (1.0, -10.8) {
  ...
  % ctetris_debug.h (shared) - Shifted right and down
  \node[class, fill=gray!8, draw=gray] (ctdbg) at (8.0, -10.8) {
...
\begin{tikzpicture}[
  ...
  store/.style={cylinder,draw=gray!60,fill=gray!10,shape border rotate=90,aspect=0.25,
                minimum width=1.8cm,minimum height=0.45cm,align=center,font=\tiny,inner sep=2pt},
  ...
  \node[extio](gist) at (0,7.5)  {Gist\\manifest+JSON};
  \node[proc] (sync) at (3.8,7.5){syncFromGist()};
  \node[store](meta) at (7.6,7.5){shared\_meta};
  \node[proc] (psc)  at (3.8,5.0){postSyncCheck()};
  \node[store](us)   at (7.6,5.0){idUser\_Stories};
  \node[store](sdat) at (7.6,2.5){shared\_data};
  \node[proc] (load) at (3.8,2.5){storyDbLoad\\Dialogue()};
  ...
  \node[extio](user) at (0,2.5)  {SDL Events};
...
\begin{VersionTable}
\Vone & Logo animation, brand identity, BUILD\_\-STANDALONE
      & SDL3, nanosvg/rast
      & Standalone $\rightarrow$ \texttt{runGameStory(win,}\allowbreak\texttt{ren)} entry \\ \hline
\Vtwo & Dialogue engine, Gist SHA-diff sync, post-sync condition check
      & sqlite3, nlohmann/json, libcurl / emscripten\_fetch
      & Reads gameConsole DB; writes \texttt{shared\_meta}, \texttt{idUser\_Stories} \\ \hline
\Vthree & Per-file media disk cache, download-speed loading bar, Cloudflare D1 + Duration Objects migration
