...
\newcommand{\ThreeLayerArch}[4]{%
  \begin{center}\begin{tikzpicture}[node distance=0.35cm]
    ...
    \node[layerBox,fill=pillarBlue!8,draw=pillarBlue!40,below=of L1](L2){#3};
    \node[layerBox,fill=gray!10,draw=gray!50,below=of L2](L3){#4};
...
\begin{center}
\begin{tikzpicture}[
  b/.style={sdsBox=pillarBlue!60,text width=2.5cm},
  dec/.style={diamond,draw=versionAmber!80,fill=versionAmber!8,
              aspect=2.6,align=center,font=\scriptsize},
  arr/.style={-{Stealth[length=4pt]},thick}
]
  \node[b](in) at (0,0) {BoardSortMode\\enum input};
  \node[b,right=0.4cm of in](b1){build\\comparator\\lambda};
  \node[b,right=0.4cm of b1](b2){\texttt{SortEngine}::\\\texttt{sort(arr, n,}\\cmp, CTX\_DEFAULT)};
  \node[dec,right=0.4cm of b2](d0){$n \leq 64$?};
  \node[b,above right=0.3cm and 0.4cm of d0](ins){Insertion\\Sort\\near-O(n)};
  \node[b,below right=0.3cm and 0.4cm of d0](intro){IntroSort\\(Quick+Heap\\+Insertion)};
  \node[b,right=2.8cm of d0](out){\texttt{boardScroll=0}\\log algo\\name};
  ...
  \draw[arr](ins.east) -- ++(0.4,0) |- (out.west);
  \draw[arr](intro.east) -- ++(0.4,0) |- (out.west);
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[
  ...
  % Căn lề tự động: Cách mép phải của d2 đúng 1.5cm
  \node[skipbox, right=1.5cm of d2](d3){skip\\(idempotent)};
  
  \node[proc](q3) at (0,-10.2){\texttt{INSERT \ldots ON CONFLICT DO UPDATE}\\ \texttt{isActivated=1}};
  \node[proc](q4) at (0,-11.4){\texttt{unlocked++}};
  \node[proc](q5) at (0,-12.6){after loop: \texttt{unlocked > 0}\\ $\rightarrow$ \texttt{dbSyncToPersistent()}};
  ...
  % ++(0.5,0) và ++(1.0,0) tạo ra 2 đường song song không đè lên nhau
  \draw[arr,gray,dashed](d1.east) -- ++(0.5,0) |- node[pos=0.25,right,font=\footnotesize]{next ch} (q1.east);
  \draw[arr,gray,dashed](d3.east) -- ++(1.0,0) |- node[pos=0.25,right,font=\footnotesize]{next ch} (q1.east);
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[
  ...
  \draw[fl](router)--node[left,font=\tiny,align=center]{popup\\flags}(flags);
  ...
  \draw[fl](sort2)--node[right,font=\tiny,align=center]{sorted\\g\_board[]}(flags.east);
  \draw[fl](flags)--node[left,font=\tiny,align=center]{render\\state}(rend2);
...
\begin{tikzpicture}[x=0.72pt,y=0.72pt,every node/.style={font=\tiny}]
  ...
  % 6 buttons  y-coords inverted: display y0=130 $\rightarrow$ canvas y = 480-130-30=320
  % (omitted)
